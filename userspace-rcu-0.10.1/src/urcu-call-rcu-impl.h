...
/***
  ...
  Resque :
    1. Detect Stalling thread
    2. Report False Quiescent State for stalling thread.
    3. Kill the thread.
***/
...
/***
  ...
  1. If # of cb's > THRESH invoke resque
  2. Store current GP's CB's into next stalled_cb list
***/
...
static void *call_rcu_thread(void *arg)
{
	...
	for (;;) {
		...
		/***
      MOVE THE GLOBAL LIST OF CALLBACKS from crdp->cbs TO cbs_tmp
    ***/
    splice_ret = __cds_wfcq_splice_blocking(&cbs_tmp_head,
		...
		/**
     IF THE GLOBAL LIST IS NOT EMPTY WAIT FOR GP COMPLETION and then execute call_backs?
    
    
    1.  What happens to the callbacks registered during that period?
      -------
      |      | CB1
      | gp1  |
      |      |
      --------
      |      | CB2
      | gp2  | synchronize_rcu by cal_rcu_thread
      |      |
      -------- cal_rcu executes
      |      | CB1
      | gp3  | CB2 ***> error!!!
    ...
    2. What happens if get woke up by call_back and it splices the list and waits for present gp 
      to to complete instead of next gp?
   ...
   3. 
    ...
    Maintain a integer variable and initialize it with 2 on stall and if(1) move the cb's to stall_list
    ...
    ***/
    if (splice_ret != CDS_WFCQ_RET_SRC_EMPTY) {
		...
		if (!rt) {
      /**
        IF THERE ARE NO CB's REGISTERED in non rt SLEEP?
      **/
